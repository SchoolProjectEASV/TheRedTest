Feature: FindAvailableWarehouse - Boundary Tests
  ...
  Scenario: Start Date == End Date
    ...
    And I have 1 warehouse with total volume of "1.0"
  ...
  Scenario: Invalid date range (start after end)
    ...
    Then an error should be returned with the message "start date cannot be later than end date"
  ...
  Scenario: Zero or negative dimension
    ...
    Then an error should be returned with the message "the 3D model has invalid dimensions (zero or negative)"
